...
#' @param d A vector of discrete scores: ordinal or integer; only used when `x` is a vector.
...
cor_polychoric <- function(
  ...
  if (!is.null(d)) {
    if (coef.only) {
      return(.poly_xy(x, d, correct = correct))
    } else {
      return(.poly_xy_full(x, d, correct = correct))
    }
  }
